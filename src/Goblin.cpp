...
void Goblin::InitSprite() {
    ...
    for (int i = 0; i <= 3; i++) {
        sprites[i].texture = Goblin::texture;
    ...
    for (int j = 0; j < 4; j ++) {
        ...
        sprites[2].AddClip({(int) spr_width * j, (int) spr_height * 6, (int) spr_width, (int) spr_height});
        sprites[3].AddClip({(int) spr_width * j, (int) spr_height * 7, (int) spr_width, (int) spr_height});
    }
...
void Goblin::Render(SDL_Renderer* renderer, int camX, int camY) {
...
Goblin::Goblin(const Goblin& g) : Enemy(g) {
...
Goblin& Goblin::operator= (const Goblin& g) {
    Enemy::operator=(g);
